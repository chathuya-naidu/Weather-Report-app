...
const TravelWeather = () => {
  ...
  const renderAlertMessage = () => {
    if (weatherData) {
      ...
      const maxTempThreshold = 28.50; // Maximum temperature threshold for sunny alert

      // Check if the temperature is within the specified thresholds
      if (tempmin < minTempThreshold) {
        return <p className='alert-message'>⚠️ Rainy weather - Please carry umberella and raincoat.</p>;
      }
      if (tempmax > maxTempThreshold) {
        return <p className='alert-message'>☀️ Sunny weather - Please carry sunglasses and sunscreen!</p>;
      }
    }
    return null;
  };
